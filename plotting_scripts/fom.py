...
def get_fom(Emin=0, mode='digital'):
    if mode == 'digital':
        N=pd.read_parquet('../data/finalData/finalData.pq/', engine='pyarrow', columns=['qdc_lg', 'qdc_sg', 'amplitude','invalid', 'channel']).query('channel==0 and invalid==False and amplitude>40').reset_index().head(1000000)
    else:
    ...
    plt.plot(x, gaus(x, P1[0], P1[1], P1[2]), ms=1, label='Fit: FWHM = %s, x$_0$=%.3f'%(round(fwhm1, 2), round(P1[1],3)))
    plt.plot(x, gaus(x, P2[0], P2[1], P2[2]), ms=1, label='Fit: FWHM = %s, x$_0$=%.3f'%(round(fwhm2, 2), round(P2[1],3)))
    plt.xlim(0,0.7)
    plt.ylim(0,8000)
