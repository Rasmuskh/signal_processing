...
plt.legend(loc='upper left')
plt.ylim(0,2000)
plt.xlabel('T (TDC channel)', fontsize=12)
...
plt.plot(x, lin(x, lin_popt[0], lin_popt[1]), label='fit: t(T) = %s$\dfrac{ns}{channel}\cdot$x + %s$ns$'%(round(lin_popt[0], 2), round(lin_popt[1], 2) ) )
plt.xlabel('T (TDC channel)', fontsize=12)
